...
A **Compute Unit** serves as a standard measurement that quantifies the computational expense incurred during an API call. It's akin to how AWS charges for compute usage. The computational cost varies depending on the intensity of the queries – some are lightweight and fast (e.g., eth_blockNumber) while others can be more demanding (e.g., large eth_getLogs queries).

To simplify the discussion around Compute Units, we've introduced the concept of Compute Unit Credit (CUC). Here, 1 billion Compute Units (CU) equates to 1 Compute Unit Credit (CUC). The cost in CU for each request across different products can be found in our [API marketplace](https://blockeden.xyz/api-marketplace).

## Implementing Pay-As-You-Go with CUC

We'll set up a prepaid account for each customer. Customers can load this account with CUC, which will then be consumed as per their API usage.
...
Credits exceeding the 0.3 CUC threshold can be withdrawn, allowing you to reclaim any unused amounts.
...
2. The conversion factor has changed: **1 Billion CU now equals 1 CUC**.
3. Rate limits have been increased five-fold for almost all plans.
